...
public class ChannelRememberModule extends Module{
	...
	public void onConnect(ConnectEvent<Bot> event) throws Exception {
		List<HashMap<String, Object>> rows = Database.select("select * from "+getFormattedTableName()+" where server='"+event.getBot().getServer()+"'");
	...
	public void onJoin(JoinEvent<Bot> event) throws Exception {
		if(event.getUser().equals(event.getBot().getUserBot()) && 
				!Database.hasRow("select * from "+getFormattedTableName()+" where server='"+event.getBot().getServer()+"' and channel='"+event.getChannel().getName()+"'")) {
			Database.insert(getFormattedTableName(), "server,channel", new String[] {event.getBot().getServer(), event.getChannel().getName()}, new boolean[] {true, true});
		}
		super.onJoin(event);
	}
