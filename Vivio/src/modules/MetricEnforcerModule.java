...
/**
 * The Class MetricEnforcerModule.
 */
public class MetricEnforcerModule extends Module {
		...
		while (matcher.find()) {
			...
			if (symbols.containsKey(fromStr)) {
				...
				passMessage(
						...
						event.getChannel(),
						event.getUser(),
						getMessage(value, from.toString(), result,
								to.toString()));
			}
		}
	}
	 ...
	 * @return the message
	 */
	public String getMessage(double original, String originalUnit,
			double converted, String convertedUnit) {
		return quirkyMessages[(int) (quirkyMessages.length * Math.random())]
				+ " Try " + String.format("%g", converted) + " "
				+ convertedUnit + " instead of "
				+ String.format("%G", original) + " " + originalUnit;
	}
