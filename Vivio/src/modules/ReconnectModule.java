...
public class ReconnectModule extends Module {
	 ...
	 * 
	 * @see
	 * org.pircbotx.hooks.ListenerAdapter#onDisconnect(org.pircbotx.hooks.events
	 * .DisconnectEvent)
	 */
	public void onDisconnect(DisconnectEvent<Bot> event) {
		...
		do {
			...
			try {
				if (event.getBot().isUsesSSL())
					event.getBot().connect(event.getBot().getServer(),
							event.getBot().getPort(),
							event.getBot().getServerPassword(),
		...
		if (!event.getBot().isConnected()) {
			System.err
					.println("Unable to connect to the same server, it is unlikely I'll ever be able to reconnect");
		} else {
			String identPass = event.getBot().getIdentifyPass();
			if (identPass != null && identPass.length() > 0)
				event.getBot().identify(identPass);
		}
	}
}
