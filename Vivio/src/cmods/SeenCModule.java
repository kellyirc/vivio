...
public class SeenCModule extends Command {
	...
	public void onMessage(MessageEvent<Bot> e) throws Exception {
		super.onMessage(e);
		// userMessages.put(event.getUser().getNick(), new
		// Message(event.getMessage(), event.getChannel().getName(),
		// System.currentTimeMillis()));
		List<HashMap<String, Object>> result = Database.select("SELECT * FROM "
			...
			Database.insert(
					...
					Database.getEnclosedString(e.getUser().getNick())
							...
							+ Database.formatTimestamp(System
									.currentTimeMillis())
							+ ","
							+ Database.getEnclosedString(e.getChannel()
									.getName()));
		} else {
			Database.execRaw("UPDATE " + getFormattedTableName()
					+ " SET message="
					+ Database.getEnclosedString(e.getMessage()) + ", time="
	...
	public void execute(Bot bot, Channel chan, User user, String message) {
		...
		for (Channel c : bot.getChannels()) {
			for (User u : bot.getUsers(c)) {
				if (u.getNick().toLowerCase().equals(args[1].toLowerCase())) {
					passMessage(bot, chan, user, "You can find " + args[1]
							+ " in " + c.getName() + " right now.");
					return;
				}
			}
		}
		...
		// passMessage(bot,chan,user,args[1]+" was last seen in "+m.getChannel()+" on "+new
		// java.util.Date(m.getTimestamp())+" saying \""+m.getMessage()+"\"");
		// }
		// }
		try {
			List<HashMap<String, Object>> result = Database
					.select("SELECT * FROM " + getFormattedTableName()
							+ " WHERE lower(nick)=" + args[1].toLowerCase());
			if (!result.isEmpty()
					&& result.get(0).get("NICK").toString().toLowerCase()
				...
				passMessage(bot, chan, user, resultNick + " was last seen in "
						+ resultChan + " on " + resultTime + " saying \""
						+ resultMessage + "\"");
			}
	...
	protected void initialize() {
		...
		try {
			Database.createTable(getFormattedTableName(),
					"nick VARCHAR(30), message VARCHAR(600), time TIMESTAMP, chan VARCHAR(30)");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
